...
- PrivateRoute component: 보호된 라우트 (예정)
...
- users 테이블: auth.uid() = user_id (예정)
- places 테이블: auth.uid() = user_id (예정)
```

## 📊 테스트 시나리오

### 단위 테스트 ✅
1. ✅ useAuth hook이 인증 상태를 올바르게 반환 (9개 테스트)
2. ✅ LoginButton이 OAuth 로그인을 올바르게 트리거 (6개 테스트)
3. ✅ UserProfile이 사용자 정보를 올바르게 표시 (3개 테스트)

### 테스트 커버리지
- **전체**: 98.11% (18개 테스트 통과)
- **useAuth hook**: 100% (Statements), 87.5% (Branches)
- **LoginButton**: 100% (모든 메트릭)
- **UserProfile**: 90.9% (Statements), 75% (Branches)
...
│   ├── LoginButton.test.tsx      # 테스트 (6개)
│   ├── UserProfile.tsx           # 사용자 프로필 표시
│   └── UserProfile.test.tsx      # 테스트 (3개)
├── hooks/
│   ├── useAuth.ts                # 인증 상태 관리 훅
│   └── useAuth.test.tsx          # 테스트 (9개)
└── index.ts                      # 모듈 exports
